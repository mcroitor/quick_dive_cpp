...
Для эффективной работы с операционной системой необходимо знать её конфигурацию. В частности, при работе с потоками выполнения полезно знать, сколько ядер есть операционной системе. Стандартная библиотека С++ предлагает следующие функции для получения информации:

- `std::thread::hardware_concurrency()` - возвращает количество ядер доступных операционной системе;
