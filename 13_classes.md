...
Переменные, объявленные в классе, называются __члена-данными__ (member data), __свойствами__ (attributes), а также __полями__ (fields), функции, объявленные в классе, называются __член-функциями__ (member functions) или __методами__ (methods). В классе можно управлять доступом к объявленным переменным и функциям, делая их доступными для пользователя или скрывая их.
