...
Формально, _класс это пользовательский тип данных, описывающий абстракцию объектов реального мира_. Экземпляр класса называются _объектом_ (вообще, любую переменную можно назвать объектом типа).
...
Переменные, объявленные в классе, называются __члена-данными__ (или __свойствами__), функции, объявленные в классе, называются __член-функциями__ (или __методами__). В классе можно управлять доступом к объявленным переменным и функциям, делая их доступными для пользователя или скрывая их.
...
{
    gameCharacter._health -= (_attack - gameCharacter._defence);
}
```
