...
Для улучшения читаемости программ можно определять псевдонимы типов. Кроме того, определение псевдонимов упрощает работу с указателями на функции, а также позволяет сокращать длинные типы, например, при использовании контейнеров STL.
...
Перечислением называется пользовательский тип данных, определённый как набор целочисленных констант. Переменная перечисления может принимать только значения, определенные в этом перечислении.
...
Константы перечислений принимают последовательные целочисленные значения, начиная с нуля. Соответственно, константа `CURRENCY::MDL` будет равна нулю, `CURRENCY::USD` и так далее. Однако, настоятельно рекомендуется использовать имена констант, определенных в перечислении, а не их числовые значения.

Имя константы перечисления указывается как `<имя типа>::<имя константы>`.
