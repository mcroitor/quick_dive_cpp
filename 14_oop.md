...
Дочерние классы образуются из базовых за счет добавления новых свойств и переопределения некоторых методов.
...
Функция, совпадающая по имени с конструктором, но начинающаяся с символа `~`, называется __деструктором__. Роль деструктора - правильное освобождение памяти при уничтожении объекта. Класс может иметь только один деструктор.
...
При реализации конструктора с параметрами поля класса, обычно, инициализируются значениями параметров при помощи списка инициализации. Список инициализации представляет собой список параметров, разделенных запятыми, заключенных в круглые скобки и инициализируемых значениями параметров.
...
Метод называется __виртуальным__, если он может быть переопределён в дочернем классе, с использованием ключевого слова `virtual`. Пример использования виртуальных функций:
...
Абстрактный класс может содержать только публичные чистые виртуальные функции. В этом случае иногда этот класс называется интерфейсом, по аналогии с другими языками ОО.
...
Полиморфизм на основе виртуальных функций, называемый также динамическим полиморфизмом, основан на возможности указателя на объект базового класса указывать на объект класса-потомка.
