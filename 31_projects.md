...
file: first.o second.o main.o
    # подключается библиотека liblove
    g++ first.o second.o main.o -o file -love
...
`CMake` — это кроссплатформенная система автоматизации сборки, которая позволяет генерировать файлы сборки для различных платформ и компиляторов. Она использует файл `CMakeLists.txt`, в котором описаны зависимости и правила сборки проекта.

`CMake` позволяет легко управлять зависимостями, а также поддерживает различные конфигурации сборки (например, Debug и Release).

Если некоторый проект `MyProject` имеет следующую структуру:
...
- требуется использовать стандарт C++20;
- использовать библиотеку `liblove`;
- использовать библиотеку `libmath`;
- исполняемый файл должен называться `app`;

Тогда `CMakeLists.txt` для данного проекта будет выглядеть следующим образом:
...
include_directories(include) # Путь к заголовочным файлам
link_directories(lib)        # Путь к внешним библиотекам (используйте только при необходимости)

file(GLOB SOURCES "src/*.cpp") # Автоматически собирает все .cpp-файлы из папки src
add_executable(app ${SOURCES})

target_link_libraries(app love math) # Подключение библиотек
```

> [!TIP]
> Если вы хотите, чтобы исполняемый файл помещался в папку `bin`, добавьте строку:
> 
> ```cmake
> set(CMAKE_RUNTIME_OUTPUT_DIRECTORY ${CMAKE_BINARY_DIR}/bin)
> ```

Сборка проекта с помощью `CMake` выполняется следующими командами:
