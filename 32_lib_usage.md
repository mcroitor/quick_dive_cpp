...
В данном случае ключ компиляции `-I` указывает путь к заголовочным файлам библиотеки, ключ `-L` указывает путь к библиотекам, а ключ `-l` указывает имя библиотеки. Пути могут быть как абсолютными, так и относительными.
...
__Неявное связывание библиотеки__ с приложением происходит при подключении заголовочного файла, а также при указании в свойствах проекта использования библиотеки. В этом случае библиотека будет загружаться в оперативную память перед запуском приложения. При запуске приложения операционная система проверяет наличие динамических библиотек, связанных с программой, по системным путям, в текущей директории или в оперативной памяти. Только после этого программа выполняется. Если хотя бы одна библиотека не была найдена, приложение завершается с соответствующей ошибкой.

При компиляции программы, использующей динамическую библиотеку, исполняемый код программы увеличивается в размере на несколько байт, так как вместо бинарного кода функции из динамической библиотеки в программу включается вызов этой функции из библиотеки, объявленный в библиотеке импорта.

### Явное связывание

Однако динамические библиотеки могут быть загружены не только при запуске программы, но и по требованию, в момент выполнения программы.

__Явное связывание библиотеки__ осуществляется с помощью функции `LoadLibrary` (`dlopen` в Unix-системах), которая загружает библиотеку в момент вызова функции. В этом случае приложение может работать без библиотеки до того момента, пока она не будет загружена функцией `LoadLibrary`.
 ...
 * @brief Пример явного связывания динамической библиотеки, Windows
...
В Unix-системах для явного связывания библиотеки с программой необходимо выполнить аналогичные действия:

```cpp
/**
 * @file dl_load_example.cpp
 * @brief Пример явного связывания динамической библиотеки, Unix
